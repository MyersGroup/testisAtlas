...
GO_enrichment <- function(component, geneNumber = 250, threshold=0.01, side="N"){
  ...
  if(side=="N"){
    top_genes <- data.table(as.matrix(SDAresults$loadings[[1]][component, ]), keep.rownames = TRUE)[order(V1)][1:geneNumber]$rn
  }else{
    top_genes <- data.table(as.matrix(SDAresults$loadings[[1]][component, ]), keep.rownames = TRUE)[order(-V1)][1:geneNumber]$rn
  }

gene_universe <- data.table(as.matrix(SDAresults$loadings[[1]][component,]), keep.rownames = TRUE)$rn
