...
gene_loading_sparsity <- ggplot(loading_sparsity, aes(threshold, Fraction, colour=Method)) +
  ...
  xlab("Scaled Threshold (threshold * max(abs(loadings)))") +
...
gonia_SDA_zoom <- tricolour_tsne(c("Gfra1","Upp1","Nanos1")) +
  xlim(c(-6,5)) + ylim(c(36,43)) +
  ggtitle("SDA Imputed Expression") + labs(y="t-SNE 2", x="t-SNE 1")

gonia_NNMF <- tricolour_tsne(c("Gfra1","Upp1","Nanos1"), predict = "NNMF") +
  facet_zoom(xlim=c(-6,5), ylim=c(36,43), zoom.size = 2) +
  ggtitle("NNMF Imputed Expression") + labs(y="t-SNE 2", x="t-SNE 1")
# ccnd2 or gfra1
# upp1 or foxf1
# ccna2 or crabp1
...
tmp <- datat[nmf_predict(c("Rhox2h"),name_extension = "_NNMF")][sda_predict(c("Rhox2h"), name_extension = "_SDA")]
...
exp_predictions <- plot_grid(gonia_SDA_zoom, gonia_NNMF, blood_SDA_vs_NNMF,
          rel_widths = c(3,4.5,3),
          ncol=3,
          labels = c("E","","F"))

left <- plot_grid(gene_loading_sparsity, compare_imputation, hormad_cell_score,
                  ncol=1,
                  rel_heights = c(4,4,5),
                  labels = c("A","B","C"))

top <- plot_grid(left, heatmapgrob, ncol=2, rel_widths = c(2,3), labels = c("","D"))

pdf("../results/NNMF_vs_SDA.pdf", height = 12, width=15)
plot_grid(top,
          exp_predictions,
