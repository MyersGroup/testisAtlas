...
loading_sparsity <- data.table(threshold,
                               t(sapply(threshold, function(x) c("NNMF"=mean(abs(nnmf_decomp$H)/max(abs(nnmf_decomp$H)) < x),
                                                                "SDA"=mean(abs(results$loadings[[1]])/max(abs(results$loadings[[1]])) < x),
...
cellscore_sparsity <- data.table(threshold,
                               t(sapply(threshold, function(x) c("PCA"=mean(abs(pcaresult$projection)/max(abs(pcaresult$projection)) < x),
                                                                 ...
                                                                 "SDA"=mean(abs(results$scores)/max(abs(results$scores)) < x),
...
```{r}
SDAtools::genome_loadings(results$loadings[[1]][5,], label_both = T, max.items = 20, label.size = 3,
...
```{r}
SDAtools::genome_loadings(results$loadings[[1]][38,], label_both = T, max.items = 20, label.size = 3,
                                     gene_locations = rna_locations, hide_unknown = T) +
...
```{r}
ggplot(datat[nmf_predict(c("Prdm9"))], aes(-PseudoTime, Prdm9)) + geom_point(stroke=0, alpha=0.3) + ggtitle("NNMF")
ggplot(datat[sda_predict(c("Prdm9"))], aes(-PseudoTime, Prdm9)) + geom_point(stroke=0, alpha=0.3) + ggtitle("SDA")
ggplot(datat[pca_predict(c("Prdm9"))], aes(-PseudoTime, Prdm9)) + geom_point(stroke=0, alpha=0.3) + ggtitle("PCA")
```
...
```{r, fig.width=6}
compare_factorisations(nnmf_decomp$H, results$loadings[[1]], names=c("NNMF","SDA"))
compare_factorisations(t(pcaresult$loadings), results$loadings[[1]], names=c("PCA","SDA"))
compare_factorisations(ica_decomp$A, results$loadings[[1]], names=c("ICA","SDA"))
```

## 100 componentns

```{r, fig.height=8, fig.width=5}
compare_factorisations(results$loadings[[1]],nnmf_decomp5$H, names=c("SDA","NNMF"), method="pearson")
```

## Pearson

```{r}
named_loadings <- results$loadings[[1]]
...
compare_factorisations(t(pcaresult$loadings), results$loadings[[1]], names=c("PCA","SDA"), method = "pearson")
compare_factorisations(ica_decomp$A, results$loadings[[1]], names=c("ICA","SDA"), method = "pearson")
```

## Rotated NNMF

```{r, fig.height=8}
rot_NNMF <- vegan::procrustes(t(results$loadings[[1]]), t(nnmf_decomp$H))
...
sda_load_promax <- promax(t(results$loadings[[1]]))
...
plot(sda_load_promax$loadings[,"V5"],results$loadings[[1]]["V5",])
plot(nnmf_load_promax$loadings[,"V50"],nnmf_decomp$H["V50",])

plot(rot_NNMF$Yrot[,"V5"], results$loadings[[1]]["V5",])
plot(nnmf_decomp$H["V50",],results$loadings[[1]]["V5",])
```
...
gene_cor <- numeric(length = length(colnames(results$loadings[[1]])))

for(i in seq_along(gene_cor)){
  gene_cor[i] <- cor(nmf_predict(c(colnames(results$loadings[[1]])[i]))[,2][[1]],
                     sda_predict(c(colnames(results$loadings[[1]])[i]))[,2][[1]])
}

names(gene_cor) <- colnames(results$loadings[[1]])
...
rownames(named_results$loadings[[1]]) <- paste0("Mix",1:50," ",names(sort(component_labels)))
colnames(named_results$scores) <- paste0("Mix",1:50," ",names(sort(component_labels)))

sc_components <- grep("Single",rownames(named_results$loadings[[1]]))

named_results$loadings[[1]] <- named_results$loadings[[1]][-sc_components,]
named_results$scores <- named_results$scores[,-sc_components]

gene_cor <- numeric(length = length(colnames(results$loadings[[1]])))

for(i in seq_along(gene_cor)){
  gene_cor[i] <- cor(nmf_predict(c(colnames(results$loadings[[1]])[i]))[,2][[1]],
                     sda_predict(c(colnames(results$loadings[[1]])[i]), factorisation = named_results)[,2][[1]])
}
...
gonia_SDA <- tricolour_tsne(c("Gfra1","Upp1","Nanos1")) +
  facet_zoom(xlim=c(-5,5), ylim=c(36,43), zoom.size = 2) +
  ggtitle("SDA Imputed Expression") + labs(y="t-SNE 2", x="t-SNE 1")

gonia_SDA_zoom <- tricolour_tsne(c("Gfra1","Upp1","Nanos1")) +
  xlim(c(-4,5)) + ylim(c(36,44)) +
  ggtitle("SDA Imputed Expression") + labs(y="t-SNE 2", x="t-SNE 1")

gonia_NNMF <- tricolour_tsne(c("Gfra1","Upp1","Nanos1"), predict = "NNMF") +
...
```{r}
#plot(nnmf_decomp$W[,"V4"][datat[order(experiment)]$cell], pch=".")
...
tmp$NNMF_V4 <- nnmf_decomp$W[,"V4"][datat$cell]
tmp$NNMF_V11 <- nnmf_decomp$W[,"V11"][datat$cell]
tmp$NNMF_V34 <- nnmf_decomp$W[,"V34"][datat$cell]
...
plot(single_component_enrichment(results$loadings[[1]][9,], etc_genes_clean, threshold = 500, pos = F)$ranks)

plot(single_component_enrichment(nnmf_decomp$H["V46",], etc_genes_clean, threshold = 500)$ranks, 
     single_component_enrichment(results$loadings[[1]][9,], etc_genes_clean, threshold = 500, pos = F)$ranks)
```


```{r}
nnmf_ranks <- single_component_enrichment(nnmf_decomp$H["V46",], etc_genes_clean, threshold = 500)$ranks
sda_ranks <- single_component_enrichment(results$loadings[[1]][9,], etc_genes_clean, threshold = 500, pos = F)$ranks
...
plot(single_component_enrichment(results$loadings[[1]][5,], kojima_stra8_chip, threshold = 500, pos = F)$ranks)
...
Looking for something NNMF does better than SDA

```{r}
plot(results$loadings[[1]][45,], nnmf_decomp$H["V48",], pch=".")
plot(results$loadings[[1]][45,], nnmf_decomp$H["V8",], pch=".")
plot(nnmf_decomp$H["V48",], nnmf_decomp$H["V8",], pch=".")

colnames(nnmf_decomp$H)[nnmf_decomp$H["V8",]>0.2 & nnmf_decomp$H["V48",]<0.001]
plot(results$loadings[[1]][49,], nnmf_decomp$H["V48",], pch=".")
plot(results$loadings[[1]][37,], nnmf_decomp$H["V48",], pch=".")

SDAtools::genome_loadings(results$loadings[[1]][45,], label_both = T, max.items = 20, label.size = 3,
                                     gene_locations = rna_locations, hide_unknown = T) +
  ylab("Gene Loadings (SDA Component 45)")

genome_loadings(nnmf_decomp$H["V48",], label_both = FALSE, max.items = 20, label.size = 3,
                                     gene_locations = rna_locations, hide_unknown = T) +
  ylab("Gene Loadings (NNMF Component 48)")

genome_loadings(nnmf_decomp$H["V8",], label_both = FALSE, max.items = 20, label.size = 3,
                                     gene_locations = rna_locations, hide_unknown = T) +
  ylab("Gene Loadings (NNMF Component 8)")

highest_components(results, "Ctsl")

SDAtools::genome_loadings(results$loadings[[1]][49,], label_both = T, max.items = 20, label.size = 3,
                                     gene_locations = rna_locations, hide_unknown = T) +
  ylab("Gene Loadings (SDA Component 49)")

```

```{r}
nnmf_decomp$H[c("V48"),(nnmf_decomp$H["V8",]<0.001 & nnmf_decomp$H["V48",]>0.1)]
#      Psap     Mfge8 Serpina3a      Aqp8      Insc 
# 0.1134520 0.1197093 0.1478793 0.1629060 0.1180692 

sort(nnmf_decomp$H["V8",]/nnmf_decomp$H["V48",])

tricolour_tsne(c("Aard","Serpina3a"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Serpina3a"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Aqp8"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Aqp8"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Psap"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Psap"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Insc"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Insc"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))

tricolour_tsne(c("Aard","Maged2"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Maged2"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Mfge8"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Mfge8"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))

tricolour_tsne(c("Aard","Insc","Psap"), predict = "NNMF") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))
tricolour_tsne(c("Aard","Serpina3a"), predict = "SDA") + facet_zoom(xlim=c(-10,5), ylim=c(10,20))

corss <- rbind(sda_predict(c("Aard","Serpina3a")), nmf_predict(c("Aard","Serpina3a")))
corss$Method <- "NNMF"
corss[1:(nrow(corss)/2)]$Method <- "SDA"

corss$rgb <- ternaryColours(corss, c("Aard","Serpina3a"))

ggplot(corss, aes(Aard, Serpina3a, colour=rgb)) + scale_colour_identity() + geom_point() + facet_wrap(~Method, scales = "free")

ggplot(corss, aes(Aard, Serpina3a, colour=Method)) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
  theme_minimal()
```
...
# plot(results$scores[datat[order(-PseudoTime)]$cell,"V38"],col=rgb(0,0,0,1-as.double(grepl("Hormad",datat[order(-PseudoTime)]$experiment))),xlim=c(0,5000),ylim=c(-5,20))
# plot(nnmf_decomp$W[datat[order(-PseudoTime)]$cell,"V3"],col=rgb(0,0,0,1-as.double(grepl("Hormad",datat[order(-PseudoTime)]$experiment))),xlim=c(0,5000))
# 
# plot(results$scores[datat[order(-PseudoTime)]$cell,"V38"],col=1+as.double(grepl("Hormad",datat[order(-PseudoTime)]$experiment)),xlim=c(0,5000),ylim=c(-5,20))
# plot(nnmf_decomp$W[datat[order(-PseudoTime)]$cell,"V3"],col=1+as.double(grepl("Hormad",datat[order(-PseudoTime)]$experiment)),xlim=c(0,5000))

# Compare Hormad1KO Cell Scores

tmp <- data.table("NNMF"=nnmf_decomp$W[datat[order(-PseudoTime)]$cell,"V3"],
           "SDA"=results$scores[datat[order(-PseudoTime)]$cell,"V38"],
           "Hormad1KO"=grepl("Hormad",datat[order(-PseudoTime)]$experiment),
           "PseudoTime" = datat[order(-PseudoTime)]$PseudoTime,
           "cell" = datat[order(-PseudoTime)]$cell,
           "msci_ratio" = datat[order(-PseudoTime)]$msci_ratio)
...
datat[grepl("Hormad",experiment), Hormad_Genotype := "Hormad -/-"]
datat[!grepl("Hormad",experiment), Hormad_Genotype := "Hormad WT"]

tmp <- datat[nmf_predict(hormad_gene,name_extension = "_NNMF")][sda_predict(hormad_gene, name_extension = "_SDA")]
...
datat[grepl("Hormad1_3",experiment), Genotype := "2"]
datat[!grepl("Hormad1_3",experiment), Genotype := "1"]

tmp <- datat[nmf_predict(c("Rhox2h"),nnmf_decomp5, name_extension = "_NNMF")][sda_predict(c("Rhox2h"), name_extension = "_SDA")]
...
# Create publication figure panel
