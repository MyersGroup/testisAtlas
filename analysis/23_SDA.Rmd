...
mean(results$pips[[1]]<0.5)

# max loadings with low PIP
max(abs(results$loadings[[1]][results$pips[[1]]<0.5]))

# range of loadings with low PIP
quantile(results$loadings[[1]][results$pips[[1]]<0.5], c(0.005,0.995))

# mean loading PIP>0.5
mean(abs(results$loadings[[1]][results$pips[[1]]>0.5]))

# Max loading
max(results$loadings[[1]][results$pips[[1]]>0.5])
...
PIP_distribution(results), labels = "AUTO")
dev.off()
...
head(sort(results$scores[,4]))

#sc_correlations <- sapply(rownames(data), function(x) cor(data[x,], results$loadings[[1]][4,], method = "spearman"))
...
c4scores <- qplot(1:length(results$scores[,4]),sort(results$scores[,4],T), geom="point") + labs(x="Sorted Cell Index", y="Component 4 Cell Loading")

v4_loadings_plot <- genome_loadings(results$loadings[[1]][4,], label_both = FALSE, max.items = 10, label.size = 3, hide_unknown = T, gene_locations = rna_locations, ) +
  ylab("Gene Loading (Component 4)")

cor1 <- qplot(data["ATCCTCACCAAN.Mlh3_1190",], c(results$scores["ATCCTCACCAAN.Mlh3_1190",4,drop=F] %*% results$loadings[[1]][4,]), alpha=I(0.3), stroke=0) + 
  annotate("text", x=1.7, y=8, label=paste("Correlation:",signif(cor(data["ATCCTCACCAAN.Mlh3_1190",], -results$loadings[[1]][4,]),3))) +
...
cor2 <- qplot(data["ATCCTCATCCAA.Mlh3_1190",], c(results$scores["ATCCTCATCCAA.Mlh3_1190",4,drop=F] %*% results$loadings[[1]][4,]), alpha=I(0.3), stroke=0) + 
  annotate("text", x=1.7, y=1, label=paste("Correlation:",signif(cor(data["ATCCTCATCCAA.Mlh3_1190",], -results$loadings[[1]][4,]),3))) +
...
sans4 <- qplot(data["ATCCTCACCAAN.Mlh3_1190",], c(results$scores["ATCCTCACCAAN.Mlh3_1190",-c(4),drop=F] %*% results$loadings[[1]][-c(4),]), alpha=I(0.3), ylim = c(-2,10.5), stroke=0) + 
  annotate("text", x=1.7, y=8, label=paste("Correlation:",signif(cor(data["ATCCTCACCAAN.Mlh3_1190",], c(results$scores["ATCCTCACCAAN.Mlh3_1190",-c(4),drop=F] %*% results$loadings[[1]][-c(4),])),3))) +
...
allcomps <- qplot(data["ATCCTCACCAAN.Mlh3_1190",], c(results$scores["ATCCTCACCAAN.Mlh3_1190",,drop=F] %*% results$loadings[[1]]), alpha=I(0.3), ylim = c(-2,10.5), stroke=0) + 
  annotate("text", x=1.7, y=8, label=paste("Correlation:",signif(cor(data["ATCCTCACCAAN.Mlh3_1190",], c(results$scores["ATCCTCACCAAN.Mlh3_1190",,drop=F] %*% results$loadings[[1]][,])),3))) +
...
set.seed(123)
tsne_scores <- Rtsne::Rtsne(abs(t(results$scores)), verbose=TRUE, pca=FALSE, perplexity=2, max_iter=5000)
...
sda_umap <- umap(abs(t(results$scores)), n_neighbors = 2, n_threads = 1)
...
ggplot(expression_dt("mt-Rnr2")[datat], aes(Tsne1_QC1, Tsne2_QC1, colour=`mt-Rnr2`)) + geom_point(stroke=0, size=1) + scale_color_viridis(direction = -1)

somatic_cells <- ggplot(datat, aes(Tsne1_QC1, Tsne2_QC1, color=somatic4)) +
  geom_point(size=0.1) +
  theme(legend.position = "bottom") +
  ggtitle("t-SNE - experiment") +
  scale_color_brewer(palette = "Set1")
...
max_component <- data.table(max_component = sapply(1:nrow(results$scores), function(x) which.max(abs(results$scores[x,]))),
                            cell = rownames(results$scores))

datat <- max_component[datat, on="cell"]
```


```{r, fig.width=6}
datat[, somatic5 := F]
datat[max_component %in% c(3,10,11,16,19,21,24,26,32,37,40,45,49) | `mt-Rnr2`>3.5 , somatic5 := T]

ggplot(datat, aes(Tsne1_QC1, Tsne2_QC1, colour=somatic4)) + geom_point(stroke=0) + theme(legend.position = "bottom")
ggplot(datat, aes(Tsne1_QC1, Tsne2_QC1, colour=somatic5)) + geom_point(stroke=0) + theme(legend.position = "bottom")
