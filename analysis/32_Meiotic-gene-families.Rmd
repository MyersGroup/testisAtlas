...
tfs <- tfs[(tfs %in% colnames(results$loadings[[1]]))]
...
cyclins <- cyclins[(cyclins %in% colnames(results$loadings[[1]]))]
...
krab_zinc_fingers <- krab_zinc_fingers[(krab_zinc_fingers %in% colnames(results$loadings[[1]]))]
str(krab_zinc_fingers)

KZF_melt <- melt_genes(datat, krab_zinc_fingers)
...
top_10_early_znf <- names(head(sort(datat[PseudoTime>(15000) , colSums(.SD, na.rm = TRUE), .SDcols = krab_zinc_fingers], T),15))
