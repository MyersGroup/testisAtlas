...
# Imputed
diff_expression <- data.table(
  odd = colMeans(results$scores[cell_data[subset=="Odd_48"]$cell,] %*% results$loadings[[1]]),
  normal = colMeans(results$scores[cell_data[subset=="Normal"]$cell,] %*% results$loadings[[1]]),
  gene = colnames(data))
...
```{r, fig.height=20, fig.width=5}
 #/ colSums(abs(results$scores[cell_data[subset=="Odd_48"]$cell,]))

tmp <- melt(data.table(results$scores[cell_data[subset!="Other"]$cell,], keep.rownames = T), id.vars = "rn", variable.name = "Component")
