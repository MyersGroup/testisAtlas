...
datat[,subset:="Other"]
datat[((Tsne1_QC1-(34))^2 + (Tsne2_QC1-(9))^2) < 5^2, subset:= "Normal"]
datat[((Tsne1_QC1-(45))^2 + (Tsne2_QC1-(9))^2) < 5^2, subset:= "Odd_48"]
...
diff_expression <- data.table(
  odd = Matrix::colMeans(data[datat[subset=="Odd_48"]$cell,]),
  normal = Matrix::colMeans(data[datat[subset=="Normal"]$cell,]),
  gene = colnames(data))

# Imputed
diff_expression <- data.table(
  odd = colMeans(results$scores[datat[subset=="Odd_48"]$cell,] %*% results$loadings[[1]]),
  normal = colMeans(results$scores[datat[subset=="Normal"]$cell,] %*% results$loadings[[1]]),
  gene = colnames(data))
...
write.table(names(head(sort(results$loadings[[1]][48,]),250)), file = "../data/48_neg_loadings.txt", row.names = F, quote = F, col.names = F)

write.table(names(head(sort(-results$loadings[[1]][48,]),250)), file = "../data/48_pos_loadings.txt", row.names = F, quote = F, col.names = F)
...
genome_loadings(results$loadings[[1]][48,], gene_locations = rna_locations)
print_gene_list(48)
...
```{r, fig.height=20, fig.width=5}
 #/ colSums(abs(results$scores[datat[subset=="Odd_48"]$cell,]))

tmp <- melt(data.table(results$scores[datat[subset!="Other"]$cell,], keep.rownames = T), id.vars = "rn", variable.name = "Component")

tmp[rn %in% datat[subset=="Odd_48"]$cell, type := "Odd"]
tmp[rn %in% datat[subset=="Normal"]$cell, type := "Normal"]
