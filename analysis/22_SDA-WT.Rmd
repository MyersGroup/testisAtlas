...
```{r}
combolm42 <- lm(results1$loadings[[1]]["Mix42 Pachytene",] ~ t(results9_WT$loadings[[1]]))
summary(combolm42)
plot(predict.lm(combolm42), results1$loadings[[1]]["Mix42 Pachytene",], pch=".")
cor(predict.lm(combolm42), results1$loadings[[1]]["Mix42 Pachytene",])
cor(-results9_WT$loadings[[1]]["WT_24",]*0.911 + results9_WT$loadings[[1]]["WT_49",]*0.875 + results9_WT$loadings[[1]]["WT_42",]*0.629, results1$loadings[[1]]["Mix42 Pachytene",])
...
tmp <- which.max(abs(cor(rot_9WT$Yrot,results1$loadings[[1]]["Mix42 Pachytene",])))
plot(rot_9WT$Yrot[,42],results1$loadings[[1]]["Mix42 Pachytene",], pch=".")
cor(rot_9WT$Yrot[,42],results1$loadings[[1]]["Mix42 Pachytene",])

tmp <- which.max(abs(cor(rot_9WT$Yrot,results1$loadings[[1]]["Mix9 Respiration",])))
plot(rot_9WT$Yrot[,9],results1$loadings[[1]]["Mix9 Respiration",], pch=".")
cor(rot_9WT$Yrot[,9],results1$loadings[[1]]["Mix9 Respiration",])
```
...
```{r, fig.width=8, fig.height=8}
cong_9WT <- psych::factor.congruence(rot_9WT$Yrot, rot_9WT$X)
...
```{r, fig.width=8, fig.height=8}
Heatmap(sort_matrix(rot_9WT$rotation, sort(apply(abs(rot_9WT$rotation), 2, max), decreasing = T)),
        cluster_rows = FALSE, cluster_columns = FALSE)
```
...
```{r}
plot(sort(rot_9WT$rotation)); abline(h=0); abline(h=1e-2, col="red"); abline(h=1e-1, col="blue"); abline(h=0.2, col="blue")

reg_rot <- rot_9WT$rotation
...
row_9WTb_list <- list(loadings = list(t(rot_9WTsparse + rnorm(prod(dim(rot_9WTsparse)), 0, 0.00001))),
...
loading_sparsity <- data.table(threshold,
                               t(sapply(threshold, function(x) c("Rotated_WT_SDA"=mean(rot_9WT$Yrot/max(abs(rot_9WT$Yrot)) < x),
                                                                 "Rotated_WT_SDA_reg"=mean(rot_9WTb/max(abs(rot_9WTb)) < x),
...
Heatmap(sort_matrix(rot_9WT$rotation, sort(apply(abs(rot_9WT$rotation), 2, max), decreasing = T)),
        col=log_colour_scale(rot_9WT$rotation, scale = 0.15),
        cluster_rows = FALSE, cluster_columns = FALSE)
dev.off()
```
...
mix_wt <- data.table(results1$loadings[[1]]["Mix31 Undifferentiated Spermatogonia",common_genes],
                                 rot_9WT$Yrot[,"WT_rot31"],
...
ggplot(datat[data.table(rot_9WT_scoresrot[,"WT_rot31", drop=F], keep.rownames = T)],
...
ggplot(datat[data.table(rot_9WT_scoresrot_sparse[,"WT_rot42", drop=F], keep.rownames = T)], aes(WT_rot42, V42, colour=PseudoTime)) + geom_point(stroke=0, size=0.5) + scale_colour_viridis()

pdf("../results/WT/Pachytene_cellscore_scatter.pdf")
ggplot(datat[data.table(rot_9WT_scoresrot[,"WT_rot42", drop=F], keep.rownames = T)],
...
ggplot(datat[data.table(results9_WT$scores[,"WT_42", drop=F], keep.rownames = T)],
...
ggplot(datat[data.table(rot_9WTsparse_scoresrot[,"WT_rot42", drop=F], keep.rownames = T)], 
...
```{r}
ggplot(datat[data.table(rot_9WT_scoresrot_sparse[,"Mix42 Pachytene", drop=F], keep.rownames = T)], 
