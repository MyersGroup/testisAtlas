...
msci_pseudotime <- ggplot(datat[somatic4==FALSE], aes(-PseudoTime, sex_predictions / autosomal_predictions, colour = V38)) +
  geom_point(size=0.25,alpha=0.5) +
  scale_color_viridis(direction=-1) +
  labs(color = "Component 38 (Hormad1 KO) Cell Scores", y="Ratio of total sex to autosomal expression") +
  theme_minimal() +
  theme(legend.position = "bottom")

msci_pseudotime

msci_pseudotime_sans38 <- ggplot(datat[somatic4==FALSE & V38<1], aes(-PseudoTime, sex_predictions / autosomal_predictions)) +
    geom_point(size=0.25,alpha=0.5) +
    labs(y="Sex to Autosomal Expression Ratio", x="Pseudotime") +
    theme_minimal()

msci_pseudotime_sans38

saveRDS(msci_pseudotime_sans38, "../data/tmp_cache/msci_pseudotime_sans38.rds")
```
...
```{r, fig.width=6}
X_escapees_plot <- ggplot(melt_genes(x_escapees$`Gene Symbol`), aes(-PseudoTime, value, group=variable)) +
  geom_smooth(method = "gam",formula = y ~ s(x, k = 25),  se=FALSE, colour="black") +
